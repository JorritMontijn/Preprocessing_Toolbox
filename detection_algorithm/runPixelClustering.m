...
imProcOrig = im2double(imread(strImage));
imProc = padarray(imProcOrig,[intSpatWin*2 intSpatWin*2 0],'symmetric','both');
...
for intPixelIdx=1:intPixTot
	...
	%if mod(intY,3) ~= 0,continue;end %skip y-pixels
	intX = vecPixX(intPixelIdx);
	%if mod(intX,3) ~= 0,continue;end %skip x-pixels
...
for intClustPixIdx = 1:intClusterPixels
	%msg
	if mod(intClustPixIdx,round(intClusterPixels/10))==0,fprintf('Now at %d/%d [%s]\n',intClustPixIdx,intClusterPixels,getTime);end
...
matEdges = imfilter(matBelongsToSameAsNeighbor,matFilter);
...
imProc2 = (imProcOrig(1:2:end,:,:) + imProcOrig(2:2:end,:,:))/2;
imProc2 = (imProc2(:,1:2:end,:) + imProc2(:,2:2:end,:))/2;
mapColor = parula(256);
vecSmallRange = 2:9;
matCentroidsAvgIm = [];
for intRadIdx=1:numel(vecSmallRange)
	...
	[centers,radii,metric] =  imfindcircles(imProc2,[intSmallRad intLargeRad],...
...
subplot(2,2,3)
imshow(imSegment);hold on;
scatter(matCentroidsAvgIm(:,1),matCentroidsAvgIm(:,2),'bx');hold off;


subplot(2,2,4)
imshow(imProc2);hold on;
scatter(matCentroidsAvgIm(:,1),matCentroidsAvgIm(:,2),'bx');hold off;
...
subplot(2,2,3)
hold on;
scatter(matCentroidsCorrMat(:,1),matCentroidsCorrMat(:,2),'rx');hold off;
	...
	for intType = [1 2]
		try
			...
			if intType == 1
				...
				%im-based
				%title('Im')
				matMap = imProc2(vecSelectY,vecSelectX);
			end
			if boolPlot
				subplot(2,2,2);
				imagesc(matMap);
...
indKeepROIs = false(1,numel(vecSortedR2Corr));
for intSortedROI=1:numel(vecSortedR2Corr)
	% get ROI idx and R2
